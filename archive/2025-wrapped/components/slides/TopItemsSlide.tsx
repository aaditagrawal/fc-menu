      ...
      <div className="text-center space-y-8 max-w-xl mx-auto">
        ...
        <div className="space-y-4 mt-8">
          {items.slice(0, 5).map((item, index) => (
            ...
            >
              <div
                className={`text-3xl font-mono font-bold ${index === 0 ? accentClass : "text-muted-foreground"}`}
              >
                {index + 1}
              </div>
        ...
        {items[0] && (
          <p
            className="text-sm text-muted-foreground mt-4 animate-fade-in delay-800"
            style={{ opacity: 0, animationFillMode: "forwards" }}
          >
            You probably had <span className={`font-medium ${accentClass}`}>{items[0].name}</span>{" "}
            on most days!
          </p>
        )}
      </div>
